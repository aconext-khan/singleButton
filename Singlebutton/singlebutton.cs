...
public class singlebutton
{
    ...
    public singlebutton()
    {
        try
        {
            ...
            theUI = UI.GetUI();
            string dirPath = Path.GetDirectoryName(theSession.ExecutingJournal);
            string fileName = Path.GetFileNameWithoutExtension(theSession.ExecutingJournal);
            string extension = ".dlx";
            theDlxFileName = string.Format("{0}\\{1}{2}", dirPath, fileName, extension);
    ...
    //Callback Name: ok_cb
    //------------------------------------------------------------------------------
    public void ObjectCollection(string dsNdsEtc, string selectionNr)
    {
        //Initialize all the required objects
        ListingWindow lw = theSession.ListingWindow;
        Part currentPart = (Part)theSession.Parts.BaseWork;

        //Set the inputs
        string executingJournalPath = Path.GetDirectoryName(theSession.ExecutingJournal);
        string stepExtension = ".stp";
        string inputFileName = currentPart.Name;
        //string dsGuid = Guid.NewGuid().ToString("N");
        string targetFileName = string.Format("\\{0}_{1}_{2}", inputFileName, dsNdsEtc, stepExtension);
        string targetFilePath = executingJournalPath + targetFileName;
        string _currentFilePath = currentPart.FullPath;
        Session _theSession = Session.GetSession();
        UI _theUi = UI.GetUI();
        NXMessageBox _msgBox = _theUi.NXMessageBox;

        //Write to the listing window
        lw.Open();
        lw.WriteFullline("Please wait while the " + dsNdsEtc + selectionNr + "is being written to file");

        //Get the selected objects
        var buttonFound = (NXOpen.BlockStyler.SelectObject)theDialog.TopBlock.FindBlock(selectionNr);
        var selection0Objects = buttonFound.GetSelectedObjects();

        //Write the objects to a list
        List<TaggedObject> theObjects = new List<TaggedObject>(selection0Objects);
        var numse1 = theObjects.Count;


        for (var i = 0; i <= numse1 - 2; i++)
        {
            //var selObj = selection0.GetSelectedObjects();
            var selObj = _theUi.SelectionManager.GetSelectedTaggedObject(i);
            if (selObj != null)
            {
                theObjects.Add(selObj);
            }
        }

        if (theObjects.Count == 0)
        {
            _msgBox.Show("Error", NXMessageBox.DialogType.Error, "No part was selected");
            return;
        }

        var stepCreator1 = _theSession.DexManager.CreateStepCreator();
        stepCreator1.ExportAs = NXOpen.StepCreator.ExportAsOption.Ap214;
        stepCreator1.SettingsFile = "C:\\CAD\\Siemens\\NX2212\\STEP214UG\\ugstep214.def";
        ...
        stepCreator1.InputFile = _currentFilePath;

        foreach (var obj in theObjects)
        {
            var added = stepCreator1.ExportSelectionBlock.SelectionComp.Add((NXObject)obj);
        }
        stepCreator1.OutputFile = targetFilePath;
    ...
    public void FaceCollectorXX(string dsNdsEtc, string faceCollectorNr)
    {
        ListingWindow lw = theSession.ListingWindow;
        Part currentPart = (Part)theSession.Parts.BaseWork;
        ...
        lw.Open();
        lw.WriteFullline("Please wait while the " + dsNdsEtc + faceCollectorNr + "is being written to file");
    ...
    public int ok_cb()
    {
        ...
        try
        {
            ...
            lw.WriteFullline("This window will close automatically after saving and synera will start it's magic subsequently!");

            ObjectCollection("DS", "selection0");
            ObjectCollection("NDS", "selection02");
            ObjectCollection("SP", "selection01");
    ...
    {
        //Construct the command to start synera headless
        string syneraFileName = "First_Workflow_AMG.syn";
        string jsonInputFileName = "input.json";
        string command = string.Format("/c syneraheadless.exe execute --file {0} --jsoninput {1}", syneraFileName, jsonInputFileName);
